...
function buildValue(node) {
  switch (node.type) {
    ...
    default: // eslint-disable-line
      console.log("buildValue => ", node.type);
      return "";
  }
}
...
function expressionStatement(node) {
  ...
  switch (expression.type) {
    ...
    default: // eslint-disable-line
      console.log("expressionStatement => ", expression.type);
      break;
  }
...
function returnStatement(node) {
  ...
  switch (arg.type) {
    ...
    default: // eslint-disable-line
      console.log("returnStatement => ", arg.type);
      break;
  }
...
function buildBlock(body) {
  ...
  let _ast = body.map((node) => {
    switch (node.type) {
      ...
      default: // eslint-disable-line
        console.log("buildBlock => ", node.type);
        return "";
    }
  });
...
function ifStatement(node) {
  ...
  if (test.type === "BinaryExpression") {
    ...
    condition = `j.binaryExpression('${operator}', j.identifier('${
      left.name
    }'), ${literal(right)})`;
